...
const Pano = () => {
  ...
  const drawCircle = (pos) => {
    ...
    if (window.event.ctrlKey || true) {
      ...
      obj.position.set(pArray[0].x * 0.95, pArray[0].y * 0.95, pArray[0].z * 0.95);
      obj.renderOrder = 999;
  ...
  return (
    <>
      <div className='control-div'>
        <div className='btn btn-success' onClick={() => {setIsNew(!isNew) }}>
          <span className="material-symbols-outlined">
            drive_folder_upload
          </span>
        </div>
        <div className={`btn btn-${status.POLYGON?'success' : 'info'}`} onClick={onPolygon.bind(this)}>
          <span className="material-symbols-outlined">
            polyline
          </span>
        </div>
        <div className={`btn btn-${status.LINE?'success' : 'info'}`} onClick={onLine.bind(this)}>
          <span className="material-symbols-outlined">
            pen_size_1
          </span>
        </div>
        ...
        {status.TEXT && <div className='form-group'>
